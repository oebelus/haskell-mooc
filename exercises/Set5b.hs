...
treeSize :: Tree a -> Int
treeSize Empty = 0
treeSize (Node t l r) = 1 + (treeSize l) + (treeSize r)
...
treeMax Empty = 0
treeMax (Node t l r) = if t > (treeMax l) then max t (treeMax r) else max (treeMax l) (treeMax r)
...
allValues f (Node t l r) = (f t) && (allValues f l) && (allValues f r)
...
cull val (Node t l r) = if val == t then Empty else (Node t (cull val l) (cull val r))
...
isOrdered tree = check tree Nothing Nothing
  where
    check Empty _ _ = True
    check (Node x left right) lower upper =
      maybe (True) (x >) lower
        && maybe (True) (x <) upper -- x > lower (if exists)
        && check left lower (Just x) -- x < upper (if exists)
        && check right (Just x) upper
...
walk s Empty = Nothing
walk (x : xs) (Node t l r) = case x of
  StepL -> walk xs l
  StepR -> walk xs r
...
set (x : xs) val (Node t l r) = case x of
  StepL -> (Node t (set xs val l) r)
  StepR -> (Node t l (set xs val r))
