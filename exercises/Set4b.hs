...
countHelper x acc = maybe (1 + acc) (\_ -> acc) x
...
maxHelper prev curr = if prev >= curr then prev else curr
...
slHelper acc x = (acc + fst x, snd x + 1)
...
largestHelper acc [] = [acc]
largestHelper acc (x : xs)
  | x == acc = acc : (x : xs)
  | x > acc = (x : xs)
  | otherwise = [acc]
