...
setName name (MkPerson age n) = (MkPerson age name)

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age (MkPerson a n) = (MkPerson age n)
...
tick :: UpDown -> UpDown
tick ud = case ud of
  Increasing x -> Increasing (x + 1)
  Decreasing x -> Decreasing (x - 1)
...
rgb col = case col of
  ...
  Mix x y ->
    let rgbx = rgb x
        rgby = rgb y
     in [mean (rgbx !! 0) (rgby !! 0), mean (rgbx !! 1) (rgby !! 1), mean (rgbx !! 2) (rgby !! 2)]
  Invert x -> [invert ((rgb x) !! 0), invert ((rgb x) !! 1), invert ((rgb x) !! 2)]
