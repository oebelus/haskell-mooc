...
oddFactorial 1 = 1
oddFactorial n = if n `mod` 2 == 1 then n * oddFactorial (n - 1) else oddFactorial (n - 1)
...
countdown x = "Ready! " ++ (countd x) ++ "Liftoff!"

countd :: Integer -> String
countd 1 = (show 1 ++ "... ")
countd x = (show x ++ "... ") ++ countd (x - 1)
...
smallestDivisor x = smallestDivisorHelp x 2

smallestDivisorHelp :: Integer -> Integer -> Integer
smallestDivisorHelp x acc
    | x `mod` acc == 0 = acc
    | otherwise = smallestDivisorHelp x (acc + 1)
